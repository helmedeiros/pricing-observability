...
- Live-stack observability verification script (`scripts/verify-registry-observability.sh` + `make verify-registry-obs`). Boots a model-registry binary on the host, drives an `/upload → /promote → /rollback` round-trip against a running markup-svc, then polls Jaeger / Prometheus / Elasticsearch HTTP APIs to assert (a) the lifecycle child spans render and markup-svc nests as a downstream service in the same trace, (b) `registry_promotions_total` ticked AND exemplars carry the `trace_id` label (requires `--enable-feature=exemplar-storage`), (c) Filebeat indexed `registry.access` events with `attrs.trace_id` populated. Operator-run (not on CI): requires the observability + platform compose stacks up + a built model-registry binary.
- Grafana dashboard `model-registry — control-plane overview` (`config/dashboards/model-registry-overview.json`). Eight panels: promotions RPS by outcome; uploads + rollbacks RPS by outcome; deploy duration p50/p95/p99 with `trace_id` exemplars plotted as dots; per-instance deploys RPS by outcome; stat tiles for promotion-failure rate (5 m), per-instance deploy-failure rate (5 m), state-drift events (5 m), and scrape target `up{job=model-registry}`. Same shape as the existing per-service overviews; the exemplar dots are clickable into Jaeger.
- Kibana saved searches for the registry runbooks (`config/kibana-saved-objects.ndjson`): `runbook: registry upload failures`, `runbook: registry promote failures`, `runbook: registry state drift`. Selectors use `msg : "registry.access" AND attrs.path : "/<path>" AND attrs.status >= 400` (and `msg : "registry.rollback.race_detected"` for drift) — fields that are actually keyword-indexed by the existing Filebeat `decode_json_fields` processor. The four registry runbooks now link to these searches the same way the markup-svc + decision-gateway + admin runbooks link to theirs.

### Changed

- `config/filebeat.yml` autodiscover regex now matches `^ghcr\.io/helmedeiros/(markup-svc|decision-gateway|traffic-gen|model-registry)`. The prior regex silently dropped the model-registry container's stdout — its events never reached Elasticsearch and the registry runbooks would have pointed at empty searches.
- `config/otel-collector-config.yaml` tail-sampling policy gains a `registry-writes` rule that keeps 100% of traces whose root span has `http.route` in `{/upload, /promote, /rollback}`. The registry's write surface is admin-class (a few operator actions per day); dropping them to the 10% probabilistic sampler would mean a mid-firing investigation could find the trace was silently discarded.
...
Per-service Prometheus scrape. The single `markup-svc` job (which routed through the gateway at `:8090/metrics`) splits into three jobs targeting each service's own `/metrics` endpoint. Closes ADR-0019.
