...
2. **Operator's trace** — Kibana: `msg:"registry.access" AND attrs.path:"/upload" AND attrs.status:>=400`. Copy a recent `attrs.trace_id` and open it in Jaeger (`http://localhost:16686/trace/<trace_id>`). The trace shows whether multipart parse, readUploadParts, or substrate Put errored.
